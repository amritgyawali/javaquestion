...
54.	Explain the difference between static and dynamic binding? Abstraction
...
77.	Is it allowed to declare main method as final? Package
...
86.	How will you use a specific Locale in Java? Serialization
...
99.	How can we create an Object dynamically at Runtime in Java? Garbage Collection
...
111.	What are the uses of Runtime class? Inner Classes
...
120.	Why do we use Static Nested interface in Java? String
...
155.	How notify() method is different from notifyAll() method? Collections
...
511.	How can you determine if your program has a deadlock? JSP
...
561.	In which scenario, Container initializes multiple JSP/Servlet objects? Java Design Patterns
...
611.	What is a Data Access Object (DAO) design pattern? Spring Questions
...
635.	What are the main components of a typical Spring based application? 636.Explain Dependency Injection (DI) concept in Spring framework?
...
921.	What is a submodule in GIT? AWS
...
943.	What are the security mechanisms available in Amazon S3? Cloud Computing
...
999.	What are the issues in using REST over HTTP for Microservices? 1000.	Can we create Microservices as State Machines?
ACKNOWLEDGMENTS
...
Properties Vector Hashtable BlockingQueue ConcurrentMap
...
AbstractSequentialList ArrayList AttributeList
...
Some of the features of BlockingQueue are: It does not accept null elements.
Its main use is in producer-consumer problems. BlockingQueue implementation is thread-safe. It can be used in inter-thread communications.
...
### 218.	Let say there is a Customer class. We add objects of Customer class to an ArrayList.
How can we sort the Customer objects in ArrayList by using customer firstName attribute of Customer class?
...
private volatile Thread threadStopper; public void start() {
...
} catch (InterruptedException e) { SOP(“Interrupted by exception!");
}
}
...
public SingletonDoubleCheck getInstance() { if (instance == null) {
...
synchronized (map) { Random r = new Random();
...
public interface B extends A{ } public interface C extends A{ } public class D implements B,C{ default void display() {
B.super.display();
}
}
...
public interface BaseOne extends BaseInterface { } public interface BaseTwo extends BaseInterface { } public class ChildClass implements BaseOne, BaseTwo { default void display(){
...
sj.add("One").add("Two").add("Three"); String desiredString = sj.toString();
...
### 406.	What does the expression
1.0 /
0.0 return? Will there be any compilation error?
...
Runtime.freeMemory() returns amount of free memory in bytes. Runtime.totalMemory() returns total memory in bytes.
...
Java Documentation says “When a thread is about to terminate due to an uncaught exception the Java Virtual Machine will query the thread	for
its UncaughtExceptionHandler usingThread.getUncaughtExceptionH will invoke the handler's uncaughtException method, passing the thread and the exception as arguments.”
...
O(n). Whereas, the insertion operation for an unordered array takes constant time of O(1).
...
### 475.	Which	protocol	does
Browser	and	Servlet	use	to
...
public static void main(String args[]){
//Converting int into Integer int count=50;
...
out refers to PrintStream class and a static member of System class. println is a method of PrintStream class.
...
_jspService() method is called to service the request. This is the core logic of JSP that generates response for request.
...
public HelloSingleton getHelloSingleton() { HelloSingleton result = helloSingleton; if (result == null) {
synchronized(this) { // Synchronize for thread safety result = helloSingleton;
if (result == null) { result = new HelloSingleton();
...
Java.lang.Class.forName() java.net.URLStreamHandlerFactory.createURLStreamHan java.util.Calendar.getInstance() java.util.ResourceBundle.getBundle() java.text.NumberFormat.getInstance() java.nio.charset.Charset.forName()
...
java.lang.reflect.Proxy java.rmi.* javax.inject.Inject javax.ejb.EJB
...
Factory Method Abstract Factory Object Pool Singleton Prototype

Structural Design Patterns: Adapter
Bridge Façade Decorator Composite Flyweight Proxy
...
ORM module Integration layers for Object Relational Mapping OXM module:	An abstraction layer	to	support Object XML mapping.
...
Web module: This provides basic web-oriented integration features. Servlet module: Support for Servlet Listeners.
WebSocket module: Support for Web Socket style messaging. Portlet module: MVC implementation for Portlet environment.
...
Creating the objects Configuring the objects
...
Implementation: Application code with implementation of APIs Aspects: Spring Aspects implemented by application
...
Create the bean Manage its lifecycle
...
163.	What are the two ways to enable RequiredAnnotationBeanPostProcesso in Spring?
...
E.g.
<beans xmlns="http://www.springframework.org/schema/beans" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.springframework.org/schema/be http://www.springframework.org/schema/beans/spring-beans- 2.5.xsd">
...
Add dependencies for Spring and Hibernate in pom.xml Implement DAO from HibernateDaoSupport
...
This is also known as advised object. In most cases it is a proxy object.
...
xsi:schemaLocation="http://www.springframework.org/schema/bea http://www.springframework.org/schema/beans/spring-beans-
3.0.xsd
...
E.g. @RequestMapping( value = "/test/mapping", method = GET,
...
@Service: This annotation marks the service classes in Spring. @Scope: We can define the scope of Spring bean by this annotation.
...
Resource: Add config file resource to Configuration object. Properties: Set properties in the Configuration object.
SessionFactory: Use Configuration object to build SessionFactory. Egg.
Configuration config = new Configuration(); config.addResource(“testInstance/configuration.hbm.xml”); config.setProperties( System.getProperties() ); SessionFactory sessions = config.buildSessionFactory();
...
...
<hibernate-mapping>
<class	name="com.testHibernate.util.Employee" table="employee" ...>
<id name="employeeId" type="java.lang.Integer">
<column name=“EMPLOYEE_ID" />
<generator class="identity" />
</id>
<property name="employeeId" type="string">
<column name="EMPLOYEE_ID" length="10" not- null="true" unique="true" />
</property>
</class>
...
query = "CALL GetEmployees(:employeeId)", resultClass = Employee.class
)
})
@Entity
...
Class is org.hibernate.criterion.Order. One example is as follows:
Egg.
...
Book book = new Book(); book.setAuthor(‘Richard’); book.setCategory(Category.MYSTERY);
...
Session s = null; Transaction trans = null; try {
s = sessionFactory.openSession(); trans = s.beginTransaction(); doTheAction(s);
trans.commit();
} catch (RuntimeException exc) { trans.rollback();
} finally { s.close();
}
...
Hashtable EHCache OSCache SwarmCache JBoss Cache 1.x JBoss Cache 2
...
This is the default fetching strategy for collections in Hibernate. Lazy fetching is better from performance perspective.
...
XML Mapping File: We can define it in XML mapping file. Egg.	<query name="findBookByAuthor”>
...
3.	Central Remote Repository: This is the central Maven repository that is located on repo.maven.apache.org or uk.maven.org or any other third party location. This where we can find artifacts from different providers that are available for download and use. Like- Hibernate, Spring libraries etc.
...
groupId: org.Orgname.Projectname artifactId: org.Orgname.Projectname version: x.x.x
...
Maven Installation directory: ${maven.home}/conf/settings.xml User Home directory: ${user.home}/ .m2 / settings.xml
...
localRepository: The value of this element is the path of this build system’s	local	repository.	The	default	value	is
...
<dependencyManagement> section. In this case, the specified POM has	to	be	replaced	with	the	dependencies	in	that	POM's
...
<dependency> section of the pom. E.g.
<dependencies>
<dependency>
...
The default locations are as follows: Source: src/main/java
Test: src/main/test Build: Target
...
GIT commit object contains following information: SHA1 name: A 40 character string to identify a commit
...
GITX-dev Gitbox Git-cola
SourceTree Git Extensions SmartGit GitUp
...
So the command to apply a stash is: git stash apply
Or we can use
...
We can use pretty option in git log command for this. git log – pretty
...
<branchname>, how does GIT know the SHA-1 of the last commit?
...
1 value. A symbolic reference contains a pointer	to another reference.
...
Exact command for this is as follows: git branch –v
It lists branches as:

issue75 83b576c fix issue
* master 7b96605 Merge branch 'issue75' testing 972ac34 add dave to the developer list
...
Index: This is the proposed next commit snapshot. Working Directory: This is the sandbox for doing changes.
...
These options tell the GIT which side to favor in merge conflicts. In ours, we run a command like git merge -Xours branchA
...
Infrastructure as a Service (IAAS): IAAS providers give low-level abstractions of physical devices. Amazon Web Services (AWS) is an example of IAAS. AWS provides EC2 for computing, S3 buckets for storage etc.
...
It means if we our current directory structure is as follows: My_dir
->Level_1_dir
...
Bourne Again shell: We use bash to run this shell. Korn shell: We can use ksh to for Korn shell.
Z shell: The command to use this is zsh C shell: We use csh to run C shell.
...
There is an overhead of HTTP in each request for communication. HTTP is not well suited for low-latency communications.
